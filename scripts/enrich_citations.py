"""
Enrich scraped PubMed abstracts with citation counts from OpenAlex.

OpenAlex is free, no API key required. We query by PMID in batches of 50.
Results are saved to data/citations.csv.

Usage:
    uv run python scripts/enrich_citations.py
"""
...
OUTPUT_CSV = os.path.join(DATA_DIR, "citations.csv")

OPENALEX_BASE = "https://api.openalex.org/works"
BATCH_SIZE = 50  # OpenAlex filter supports up to ~50 pipe-separated IDs
POLITE_EMAIL = "christoph.daube@gmail.com"  # Gets you into the "polite pool" (faster)


def fetch_citations_batch(pmids: list[str]) -> dict[str, int]:
    """Fetch citation counts for a batch of PMIDs from OpenAlex."""
    # OpenAlex filter: ids.pmid:12345|67890
    ...
    for work in data.get("results", []):
        ...
        # pmid_url looks like "https://pubmed.ncbi.nlm.nih.gov/12345678"
...
def enrich():
    ...
    # Resume support: skip already-fetched PMIDs
    already_done = set()
    existing_rows = []
    ...
    for batch in tqdm(batches, desc="OpenAlex"):
        ...
        # Save every 100 batches (~5000 papers)
        if len(all_results) % (BATCH_SIZE * 100) < BATCH_SIZE:
            pd.DataFrame(all_results).to_csv(OUTPUT_CSV, index=False)

        # Polite rate limiting: ~10 req/s is fine for polite pool
        time.sleep(0.1)
